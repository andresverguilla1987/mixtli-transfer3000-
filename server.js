...
// + Fix esquema AUTO + índices parciales (bugs corregidos en strings y USER_ID_SQLTYPE)
...
// + Password opcional en links (links.password_hash/salt/hint) + /f/:id + /api/link/:id/url
...
function rateLimit (key, limit, windowMs) {
  const now=Date.now(); const arr=RL.get(key)||[]
  const fresh=arr.filter(ts=>now-ts<windowMs); fresh.push(now)
  RL.set(key,fresh)
...
async function hashPw(pw) {
  ...
  const key = await scryptAsync(String(pw), salt, 64)
  return { salt, hash: Buffer.from(key).toString('hex') }
}
async function verifyPw(pw, salt, hash) {
  if (!hash) return true // link sin password
  const key = await scryptAsync(String(pw||''), String(salt||''), 64)
  return Buffer.from(key).toString('hex') === String(hash)
}

// ---- email (SendGrid/SMTP opcional) ----
let smtpTransport = null
if (SMTP_HOST && SMTP_USER && SMTP_PASS) {
  smtpTransport = nodemailer.createTransport({
    host: SMTP_HOST, port: parseInt(SMTP_PORT||'587',10), secure: false, auth: { user: SMTP_USER, pass: SMTP_PASS }
  })
}
async function sendOtp(email, code) {
  if (SENDGRID_API_KEY && SENDGRID_FROM) {
    ...
    const r = await fetch('https://api.sendgrid.com/v3/mail/send',{
      method:'POST', headers:{'Authorization':`Bearer ${SENDGRID_API_KEY}`,'Content-Type':'application/json'},
      body: JSON.stringify(body)
    })
    if(!r.ok) console.warn('[OTP] SendGrid', await r.text())
    return
  }
  if (smtpTransport && (SMTP_FROM || email)) {
    await smtpTransport.sendMail({
      from: SMTP_FROM || SMTP_USER, to: email, subject:'Tu código Mixtli',
...
// ---- DB init (fix strings + USER_ID_SQLTYPE real) ----
...
app.use(cors({
  ...
  allowedHeaders:['Content-Type','Authorization','x-mixtli-token','x-admin-token','x-cron-token']
}))
...
app.post('/api/auth/register', async (req,res)=>{
  try{
    ...
    if(!rateLimit('auth:'+ip, parseInt(RL_AUTH_PER_MIN), 60*1000)) return res.status(429).json({ error:'rate_limited' })
    ...
    if(email) await sendOtp(email.toLowerCase(), code)
...
app.post('/api/auth/verify-otp', async (req,res)=>{
  ...
  try{
    ...
    let row
    if(email){
      const r=await c.query(`INSERT INTO users (email,plan) VALUES ($1,'FREE')
        ON CONFLICT (email) DO UPDATE SET updated_at=now()
        RETURNING id,email,phone,plan,plan_expires_at`,[email.toLowerCase()])
      row=r.rows[0]
    } else {
      const r=await c.query(`INSERT INTO users (phone,plan) VALUES ($1,'FREE')
        ON CONFLICT (phone) DO UPDATE SET updated_at=now()
        RETURNING id,email,phone,plan,plan_expires_at`,[phone])
      row=r.rows[0]
    }
    await c.query('COMMIT')
    const token=signToken(row)
    res.json({ token, user:row })
  } catch(e){ await c.query('ROLLBACK'); console.error(e); res.status(500).json({ error:'verify_failed' }) }
  finally{ c.release() }
})
...
app.post('/api/plan/upgrade', authRequired, async (req,res)=>{
  try{
    ...
    const days=(plan==='PROMAX')?parseInt(PROMAX_PERIOD_DAYS||'30'):parseInt(PRO_PERIOD_DAYS||'30')
    const { rows }=await pool.query(`
      UPDATE users SET plan=$1, plan_expires_at = now() + ($2||' days')::interval, updated_at=now()
      WHERE id=$3 RETURNING id,email,plan,plan_expires_at
    `, [plan, days, req.user.id])
...
app.post('/api/plan/downgrade', authRequired, async (req,res)=>{
  try{
    const { rows }=await pool.query(`
      ...
      WHERE id=$1 RETURNING id,email,plan,plan_expires_at
    `, [req.user.id])
    res.json({ ok:true, user: rows[0] })
  }catch(e){ console.error(e); res.status(500).json({ error:'downgrade_failed' }) }
})

// ---- presign (con password opcional)
app.post('/api/presign', authOptional, async (req,res)=>{
  try{
    const user=req.user
    const ip=clientIp(req)
    if(!user && !rateLimit('presign:'+ip, parseInt(RL_PRESIGN_PER_MIN), 60*1000))
      return res.status(429).json({ error:'rate_limited' })

    let { filename, contentType, contentLength, plan, durationDays, linkPassword, linkPasswordHint } = req.body||{}
    ...
    const ttlDays = (plan==='PRO')
      ? PROCFG.ttlDays
      : (plan==='PROMAX' ? PMCFG.ttlDays : clamp(parseInt(durationDays||FREECFG.ttlDefaultDays), FREECFG.ttlDefaultDays, FREECFG.ttlMaxDays))

    // hashing password si viene
    let pw = null
    if (linkPassword && String(linkPassword).trim() !== '') {
      pw = await hashPw(linkPassword)
    }
    ...
    if(plan==='FREE' && !user){
      ...
      const ins = await pool.query(`
        INSERT INTO links (user_id,anon_ip,plan,key,filename,content_type,size_bytes,expires_at,active,password_hash,password_salt,password_hint)
        VALUES (NULL,$1,'FREE',$2,$3,$4,$5, now()+($6||' days')::interval, true,$7,$8,$9)
        RETURNING id
      `,[anon,key,filename,contentType,contentLength,ttlDays, pw?.hash||null, pw?.salt||null, linkPasswordHint||null])
      ...
      const uploadUrl=await getSignedUrl(s3,put,{expiresIn:parseInt(UPLOAD_URL_TTL_SECONDS||'3600')})

      if (pw) {
        const base = (PUBLIC_BASE_URL || '').replace(/\/+$/,'') || ''
        const publicUrl = `${base}/f/${linkId}`
        return res.json({ id:linkId, key, protected:true, uploadUrl, uploadHeaders:{'Content-Type':contentType}, publicUrl, expiresInSeconds: ttlDays*24*3600 })
      }

      const get=new GetObjectCommand({ Bucket:SEALED.S3_BUCKET, Key:key, ResponseContentDisposition:`attachment; filename="${filename}"` })
      const downloadUrl=await getSignedUrl(s3,get,{expiresIn:parseInt(DOWNLOAD_URL_TTL_SECONDS_MAX||'86400')})
      return res.json({ id:linkId, key, protected:false, uploadUrl, uploadHeaders:{'Content-Type':contentType}, downloadUrl, expiresInSeconds: ttlDays*24*3600 })
    }
    ...
    if(plan==='PRO' && usedBytes+contentLength>PROCFG.capBytesPerPeriod)
    ...
    const ins = await pool.query(`
      INSERT INTO links (user_id,plan,key,filename,content_type,size_bytes,expires_at,active,password_hash,password_salt,password_hint)
      VALUES ($1,$2,$3,$4,$5,$6, now()+($7||' days')::interval, true,$8,$9,$10)
      RETURNING id
    `,[user.id,plan,key,filename,contentType,contentLength,ttlDays, pw?.hash||null, pw?.salt||null, linkPasswordHint||null])
    const linkId = ins.rows[0].id
    ...
    const get=new GetObjectCommand({ Bucket:SEALED.S3_BUCKET, Key:key, ResponseContentDisposition:`attachment; filename="${filename}"` })
    const downloadUrl=await getSignedUrl(s3,get,{expiresIn:parseInt(DOWNLOAD_URL_TTL_SECONDS_MAX||'86400')})
    res.json({ id:linkId, key, protected:false, uploadUrl, uploadHeaders:{'Content-Type':contentType}, downloadUrl, expiresInSeconds: ttlDays*24*3600 })
  }catch(e){ console.error(e); res.status(500).json({ error:'presign_failed', detail:String(e) }) }
})

// ---- link URL (valida password si existe)
app.post('/api/link/:id/url', async (req,res)=>{
  try{
    const id = parseInt(req.params.id,10)
    if(!id) return res.status(400).json({ error:'bad_id' })
    const { rows } = await pool.query(`
      SELECT key, filename, expires_at, active, password_hash, password_salt
      FROM links WHERE id=$1
    `, [id])
    if(!rows.length) return res.status(404).json({ error:'not_found' })
    const r = rows[0]
    if(!r.active) return res.status(404).json({ error:'inactive' })
    if(new Date(r.expires_at) < new Date()) return res.status(410).json({ error:'expired' })

    if (r.password_hash) {
      const ok = await verifyPw(req.body?.password||'', r.password_salt, r.password_hash)
      if(!ok) return res.status(401).json({ error:'bad_password' })
    }

    const get=new GetObjectCommand({ Bucket:SEALED.S3_BUCKET, Key:r.key, ResponseContentDisposition:`attachment; filename="${r.filename}"` })
    const url=await getSignedUrl(s3,get,{expiresIn:parseInt(DOWNLOAD_URL_TTL_SECONDS_MAX||'86400')})
    res.json({ url })
  }catch(e){ console.error(e); res.status(500).json({ error:'link_url_failed' }) }
})

// ---- página pública /f/:id (form pw → POST /api/link/:id/url → redirect)
app.get('/f/:id', async (req,res)=>{
  try{
    const id = parseInt(req.params.id,10)
    if(!id) return res.status(400).send('bad_id')
    const { rows } = await pool.query(`
      SELECT filename, expires_at, active, password_hash, password_hint
      FROM links WHERE id=$1
    `, [id])
    if(!rows.length) return res.status(404).send('Not found')
    const r = rows[0]
    if(!r.active) return res.status(404).send('Inactive')
    if(new Date(r.expires_at) < new Date()) return res.status(410).send('Expired')

    res.setHeader('Content-Type','text/html; charset=utf-8')
    const hasPw = !!r.password_hash
    const hint = r.password_hint ? `<small>Pista: ${r.password_hint}</small>` : ''

    res.end(`<!doctype html><html><head><meta charset="utf-8"/><meta name="viewport" content="width=device-width,initial-scale=1"/>
<title>Mixtli Link #${id}</title>
<style>:root{ color-scheme:dark } body{font-family:system-ui;background:#0b0b0c;color:#e6e6e6;margin:0}
.box{max-width:560px;margin:80px auto;padding:16px;border:1px solid #2b2b2b;border-radius:14px;background:#121212}
input,button{background:#1a1a1a;border:1px solid #333;color:#eee;border-radius:10px;padding:10px}
button{cursor:pointer} small{opacity:.8}</style></head>
<body><div class="box"><h3>Descargar: ${r.filename}</h3>
${hasPw ? `<p>Este enlace está protegido con contraseña.</p>` : `<p>Preparando descarga…</p>`}
<div id="pwBox" style="${hasPw?'':'display:none'}">
<input id="pw" type="password" placeholder="Contraseña" style="width:100%;margin-bottom:8px"/>
${hint}
<div style="margin-top:10px"><button id="go">Descargar</button></div>
</div>
<p id="msg" style="margin-top:10px"></p>
</div>
<script>
async function getUrl(pw){
  const r = await fetch('/api/link/${id}/url',{method:'POST',headers:{'Content-Type':'application/json'},body: JSON.stringify({password: pw||''})})
  if(!r.ok){ document.getElementById('msg').textContent='Contraseña incorrecta o enlace inválido'; return }
  const j = await r.json()
  location.href = j.url
}
${hasPw ? `document.getElementById('go').onclick = ()=> getUrl(document.getElementById('pw').value)` : `getUrl('')`}
</script>
</body></html>`)
...
app.put('/api/package/set-limit', authRequired, async (req,res)=>{
  try{
    const { packageId, downloadLimit } = req.body || {}
...
app.post('/api/package/presign', authOptional, async (req,res)=>{
  try{
    ...
    await pool.query(`INSERT INTO package_items (package_id,key,filename,content_type,size_bytes) VALUES ($1,$2,$3,$4,$5)`,
...
app.get('/admin/metrics', async (req, res) => {
  ...
  try {
    const [{ rows: u1 }, { rows: l1 }, { rows: p1 }] = await Promise.all([
...
app.post('/tasks/cleanup', async (req, res) => {
  ...
  try {
    ...
    const r2 = await pool.query(`UPDATE links SET active=false WHERE active=true AND expires_at < now()`)
    const r3 = await pool.query(`DELETE FROM otps WHERE exp < now() - INTERVAL '1 day'`)
    res.json({ ok:true, downgraded: r1.rowCount, linksInactivated: r2.rowCount, otpsDeleted: r3.rowCount, time: new Date().toISOString() })
  } catch (e) { console.error(e); res.status(500).json({ error:'cleanup_failed' }) }
})

await initDb()
app.listen(PORT, ()=> console.log('MixtliTransfer3000 v2.4.0 on :' + PORT))
