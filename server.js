// MixtliTransfer3000 — Sealed Server + Anon FREE
...
if (!SEALED.S3_ENDPOINT || !SEALED.S3_BUCKET || !S3_ACCESS_KEY_ID || !S3_SECRET_ACCESS_KEY) { console.error('[FATAL] Missing R2 credentials or sealed config'); process.exit(1) }
...
async function authOptional(req,_res,next){ const a=req.headers.authorization||''; const t=a.startsWith('Bearer ')?a.slice(7):''; if(!t) return next(); try{ const p=jwt.verify(t,JWT_SECRET); const { rows }=await pool.query('SELECT id,email,phone,plan FROM users WHERE id=$1',[p.uid]); req.user=rows[0]||null }catch(_e){ req.user=null } finally{ next() } }
...
function hashIp(ip){ return require('crypto').createHash('sha256').update(String(ip)).digest('hex').slice(0,32) }
...
app.post('/api/presign', authOptional, async (req,res)=>{
  try{
    ...
    if(plan==='FREE' && !user){
      ...
      const c=await pool.connect(); try{ await c.query('BEGIN'); await c.query(`INSERT INTO links (user_id,anon_ip,plan,key,filename,content_type,size_bytes,expires_at,active) VALUES (NULL,$1,'FREE',$2,$3,$4,$5, now()+($6||' days')::interval, true)`,[anon,key,filename,contentType,contentLength,ttlDays]); await c.query('COMMIT') } catch(e){ await c.query('ROLLBACK'); throw e } finally{ c.release() }
      const put=new PutObjectCommand({ Bucket:SEALED.S3_BUCKET, Key:key, ContentType:contentType, Metadata:{'x-plan':'FREE','x-origin':'mixtli'} }); const uploadUrl=await getSignedUrl(s3,put,{expiresIn:num(UPLOAD_URL_TTL_SECONDS)})
      const get=new GetObjectCommand({ Bucket:SEALED.S3_BUCKET, Key:key, ResponseContentDisposition:`attachment; filename="${filename}"` }); const downloadUrl=await getSignedUrl(s3,get,{expiresIn:num(DOWNLOAD_URL_TTL_SECONDS_MAX)})
    ...
    const usedBytes=Number(agg[0].bytes||0); const usedCount=Number(agg[0].count||0)
    if(plan==='FREE'){ if(usedCount>=FREECFG.maxLinks30d) return res.status(429).json({ error:'free_link_count_exceeded', limit: FREECFG.maxLinks30d }) }
    else if(plan==='PRO'){ if(usedBytes+contentLength>PROCFG.capBytesPerPeriod) return res.status(429).json({ error:'pro_bytes_quota_exceeded', limitBytes: PROCFG.capBytesPerPeriod, usedBytes }) }
    ...
    const c=await pool.connect(); try{ await c.query('BEGIN'); await c.query(`INSERT INTO links (user_id,plan,key,filename,content_type,size_bytes,expires_at,active) VALUES ($1,$2,$3,$4,$5,$6, now()+($7||' days')::interval, true)`,[user.id,plan,key,filename,contentType,contentLength,ttlDays]); await c.query('COMMIT') } catch(e){ await c.query('ROLLBACK'); throw e } finally{ c.release() }
    const put=new PutObjectCommand({ Bucket:SEALED.S3_BUCKET, Key:key, ContentType:contentType, Metadata:{'x-plan':plan,'x-origin':'mixtli'} }); const uploadUrl=await getSignedUrl(s3,put,{expiresIn:num(UPLOAD_URL_TTL_SECONDS)})
    const get=new GetObjectCommand({ Bucket:SEALED.S3_BUCKET, Key:key, ResponseContentDisposition:`attachment; filename="${filename}"` }); const downloadUrl=await getSignedUrl(s3,get,{expiresIn:num(DOWNLOAD_URL_TTL_SECONDS_MAX)})
