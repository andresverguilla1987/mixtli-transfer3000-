...
// Esta versión corrige el 404 de /share/:id construyendo el link del paquete
// con la base del BACKEND (APP_BASE_URL o el host del request) en lugar de PUBLIC_BASE_URL.
...
function normalizePhone(p){ if(!p) return ''; let s=String(p).replace(/[\s\-\(\)]/g,''); if(s.toLowerCase().startsWith('whatsapp:')) s=s.slice('whatsapp:'.length); if(!s.startswith && !s.startsWith('+') && /^\d{10,15}$/.test(s)) s='+'+s; return s }
async function sendSmsOnly(rawTo,text){ const to=normalizePhone(rawTo); if(!twilioClient){ console.log('[SMS:demo]',to,text); return } if(!TWILIO_FROM){ console.warn('[SMS] Falta TWILIO_FROM'); return } try{ const msg=await twilioClient.messages.create({to,from:TWILIO_FROM,body:text}); console.log('[Twilio SID]',msg.sid,'status=',msg.status) }catch(e){ console.warn('[SMS ERROR]', e?.code||e?.status||'', e?.message||String(e)) } }
...
async function buildPublicUrl(key){
  ...
  return FORCE_PATH ? `${S3_ENDPOINT.replace(/\/+$/,'')}/${S3_BUCKET}/${key}` : `https://${S3_BUCKET}.${host}/${key}`
}
...
const corsMw=cors({ origin:(o,cb)=>{ if(!o) return cb(null,true); if(ORIGINS.includes(o)||isNetlifyPreview(o)) return cb(null,true); return cb(new Error('origin_not_allowed')) }, methods:['GET','POST','PUT','DELETE','OPTIONS'], allowedHeaders:['Content-Type','Authorization','x-admin-token','x-cron-token','x-mixtli-token'], optionsSuccessStatus:204 })
...
app.get('/api/health',(_req,res)=>res.json({ok:true,time:new Date().toISOString(),ver:'2.13.1',channel:'sms-only'}))
...
app.post('/api/auth/register', otpLimiter, async (req,res)=>{
  try{
    let {email,phone}=req.body||{}; email=email?String(email).trim().toLowerCase():''; phone=phone?String(phone).trim():''
    const id=email||phone; if(!id) return res.status(400).json({error:'email_or_phone_required'})
...
app.post('/api/auth/verify-otp', async (req,res)=>{
  try{
    let {email,phone,otp}=req.body||{}; email=email?String(email).trim().toLowerCase():''; phone=phone?String(phone).trim():''
    const id=email||phone; if(!id||!otp) return res.status(400).json({error:'need_id_and_otp'})
    const ok=await verifyOtpDb(id,otp); if(!ok) return res.status(400).json({error:'otp_invalid'})
    let row
    if(email){
      row=(await pool.query(`INSERT INTO users (email, plan) VALUES ($1,'FREE') ON CONFLICT (email) DO UPDATE SET updated_at=now() RETURNING id,email,phone,plan,plan_expires_at`,[email])).rows[0]
    }else{
      row=(await pool.query(`INSERT INTO users (phone, plan) VALUES ($1,'FREE') ON CONFLICT (phone) DO UPDATE SET updated_at=now() RETURNING id,email,phone,plan,plan_expires_at`,[phone])).rows[0]
    }
...
app.post('/api/pack/create', requireAuth, async (req,res)=>{
  try{
    ...
    files.forEach(f=>{ params.push(pid,f.key,f.name||null,f.size||0,f.type||null); values.push(f"(${{params.length-4}},${{params.length-3}},${{params.length-2}},${{params.length-1}},${{params.length}})") })
    await pool.query(`INSERT INTO package_files (package_id, key, filename, size, content_type) VALUES ${values.join(',')}`, params)
    const base=(APP_BASE_URL || backendBaseFromReq(req)).replace(/\/+$/,'')
    const url=`${base}/share/${pid}`
...
app.get('/api/pack/:id', async (req,res)=>{
  try{
    ...
    const files=await Promise.all(f.rows.map(async r=>({ name:r.filename||'file', size:r.size, type:r.content_type, url: await buildPublicUrl(r.key) })))
    res.json({ id, title:p.rows[0].title, total_size:p.rows[0].total_size, expires_at:p.rows[0].expires_at, files })
...
app.get('/share/:id', async (req,res)=>{
  try{
    ...
    const items=await Promise.all(f.rows.map(async r=>{
      const url=await buildPublicUrl(r.key); const name=r.filename||'file'; const mb=(Number(r.size||0)/1048576).toFixed(2)
