// Mixtli Transfer — Backend v2.14.6-lock+wt+migrate
// Password opcional, TTL, max descargas, conteo descargas, rate-limit IP
// ZIP robusto (GetObject SDK + fallback fetch->NodeStream).
// /api/pack/create devuelve URL relativa "/share/:id". ABS para ZIP con BACKEND_PUBLIC_ORIGIN.
...
// 🔧 Auto-migración para nuevas columnas
async function migrateDb () {
  try {
    ...
    `);
    await pool.query('CREATE INDEX IF NOT EXISTS pkg_dl_pkg_idx ON package_downloads(package_id)');
    console.log('[DB] migrate ok');
  } catch (e) {
    console.warn('[DB] migrate warn:', e?.message || e)
  }
}
...
function planLimits(plan = 'FREE') {
  ...
  if (p === 'PRO') return {
    maxTotalMB: toMB(PRO_MAX_TOTAL_MB || 20480),
    maxDownloads: Number(PRO_MAX_DOWNLOADS || 1000)
  }
  return {
    maxTotalMB: toMB(FREE_MAX_TOTAL_MB || 200),
    maxDownloads: Number(FREE_MAX_DOWNLOADS || 50)
  }
}
...
app.get('/api/health', (_req, res) =>
  res.json({ ok: true, time: new Date().toISOString(), ver: '2.14.6-lock+wt+migrate', channel: 'sms-only' }))
...
app.post('/api/pack/create', requireAuth, async (req, res) => {
  try {
    ...
    if (totalMB > effectiveMaxMB) {
      return res.status(400).json({ error: 'package_too_big', max_mb: effectiveMaxMB, got_mb: totalMB })
    }
        ...
        (function(){
          ...
          btn?.addEventListener('click', async () => {
            ...
            try {
              ...
              if (!r.ok) {
                alert('Contraseña incorrecta o límite alcanzado.');
                return;
              }
...
// Guard de acceso a ZIP (password + límites + rate)
async function guardPackageAccess(req, res, next) {
  try {
    ...
    console.error('[guardPackageAccess]', e)
...
app.get('/api/pack/:id/zip', guardPackageAccess, async (req, res) => {
  try {
    ...
    archive.on('end', async () => {
      try { await pool.query('UPDATE packages SET download_count = download_count + 1 WHERE id=$1', [id]) } catch {}
    })
