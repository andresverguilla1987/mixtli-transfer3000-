// MixtliTransfer3000 Backend v2.2.1 — OTP en DB + Auth + Upgrade PRO/PROMAX + Packages (no-FK)
...
// ---------- DB init (no FKs) + cols de plan + OTP table ----------
async function initDb () {
  ...
  console.log('[DB] ready (no-FK, users.plan + plan_expires_at + otps)')
}
...
async function authOptional (req, _res, next) {
  ...
  try {
    ...
    await ensureFreshPlan(p.uid)
    const { rows } = await pool.query('SELECT id,email,phone,plan,plan_expires_at FROM users WHERE id=$1', [p.uid])
...
app.post('/api/auth/register', async (req, res) => {
  try {
    ...
    console.log('[OTP]', id, code) // demo
    res.json({ ok: true, msg: 'otp_sent' })
  } catch (e) {
    console.error(e); res.status(500).json({ error: 'otp_send_failed' })
  }
})

// Verificar OTP desde DB
async function verifyOtpDb (key, code) {
  ...
  await pool.query(`DELETE FROM otps WHERE id=$1`, [row.id]) // burn it
  return true
}
...
app.post('/api/auth/verify-otp', async (req, res) => {
  ...
  try {
    ...
    await c.query('ROLLBACK'); console.error(e); res.status(500).json({ error: 'verify_failed' })
  } finally {
    c.release()
  }
})
...
app.post('/api/plan/upgrade', authRequired, async (req, res) => {
  try {
    ...
    const days = (plan === 'PROMAX') ? num(PROMAX_PERIOD_DAYS || '30') : num(PRO_PERIOD_DAYS || '30')
...
app.post('/api/presign', authOptional, async (req, res) => {
  try {
    ...
    // aplicar plan actual del usuario autenticado
    if (user && plan !== (user.plan || 'FREE')) plan = user.plan || 'FREE'

    const ttlDays = plan === 'PRO' ? PROCFG.ttlDays
      : (plan === 'PROMAX' ? PMCFG.ttlDays
        : clamp(num(durationDays || FREECFG2.ttlDefaultDays), FREECFG2.ttlDefaultDays, FREECFG2.ttlMaxDays))
    ...
    if (plan === 'FREE' && !user) {
      ...
      const { rows } = await pool.query(
        `SELECT COUNT(*)::int AS cnt FROM links WHERE anon_ip=$1 AND created_at>= now()-INTERVAL '30 days'`, [anon]
      )
...
app.post('/api/package/create', authOptional, async (req, res) => {
  try {
    ...
    let anon = null; let uid = null
    if (user) uid = user.id
    else anon = hashIp(clientIp(req))
...
await initDb()
app.listen(PORT, () => console.log('MixtliTransfer3000 v2.2.1 on :' + PORT))
