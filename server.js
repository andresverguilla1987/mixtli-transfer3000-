// MixtliTransfer3000 — Backend planes FREE / PRO / PROMAX
// Express + Postgres + JWT + R2 (Cloudflare S3-compatible)
// by cocho mode 😈
...
const {
  ...
  // FREE
  FREE_MAX_UPLOAD_MB = '3584',               // 3.5 GB
  ...
  PRO_PERIOD_DAYS = '30',
  PRO_LINK_TTL_DAYS = '7',
  PRO_MAX_UPLOAD_MB = '',                    // opcional: vacío = sin tope por archivo

  // PROMAX
  PROMAX_PERIOD_DAYS = '30',
  PROMAX_LINK_TTL_DAYS = '22',
  PROMAX_MAX_UPLOAD_MB = '',                 // opcional

  // Presigns
  UPLOAD_URL_TTL_SECONDS = '3600',           // 1h
  DOWNLOAD_URL_TTL_SECONDS_MAX = '86400'     // 24h (firmas GET)
} = process.env

if (!S3_ENDPOINT || !S3_BUCKET || !S3_ACCESS_KEY_ID || !S3_SECRET_ACCESS_KEY) {
  console.error('[FATAL] Missing R2 env vars')
  process.exit(1)
}
if (!DATABASE_URL) {
...
// key -> { code, exp: ms, channel: 'email'|'phone' }
function setOtp(key) {
  const code = String(Math.floor(100000 + Math.random() * 900000)) // 6 dígitos
  const exp = Date.now() + num(OTP_TTL_MIN) * 60 * 1000
  otpStore.set(key, { code, exp })
  console.log('[OTP]', key, code, '(válido', OTP_TTL_MIN, 'min)')
}
...
function extOf(filename = '') {
  const i = filename.lastIndexOf('.'); return i > -1 ? filename.slice(i + 1).toLowerCase() : ''
}
...
app.post('/api/auth/register', async (req, res) => {
  ...
  setOtp(idKey)
  // En producción: envía OTP por email/SMS (SendGrid/Twilio), aquí solo log
  return res.json({ ok: true, msg: 'otp_sent' })
})

// 2) Verificar OTP → crear/leer usuario + token
...
app.get('/api/me', authRequired, async (req, res) => {
  ...
  // últimos 15 links (con presign GET corto)
...
app.post('/api/presign', authRequired, async (req, res) => {
  try {
    ...
    // En este diseño: TODOS requieren registro (FREE/PRO/PROMAX)
    // Si quisieras FREE anónimo, aquí podrías permitir sin auth con un toggle.

    // límites por plan
    const lim = planLimits(plan, durationDays)
    if (!filename || !contentType || !contentLength) return res.status(400).json({ error: 'bad_params' })
    if (Number.isFinite(lim.sizeCap) && contentLength > lim.sizeCap) {
      return res.status(413).json({ error: 'file_too_large', maxBytes: lim.sizeCap })
    }

    // Cuotas por usuario (30d rolling)
    const { rows: agg } = await pool.query(
      `SELECT COALESCE(SUM(size_bytes),0) AS sum_bytes, COUNT(*) AS cnt
       FROM links WHERE user_id=$1 AND created_at >= now() - INTERVAL '30 days'`,
      [user.id]
    )
    const usedBytes = Number(agg[0].sum_bytes || 0)
    const usedCount = Number(agg[0].cnt || 0)

    if (plan === 'FREE') {
      if (usedCount >= lim.maxLinks30d) return res.status(429).json({ error: 'free_link_count_exceeded', limit: lim.maxLinks30d })
      // no cap de bytes acumulados para FREE, solo por link
    } else if (plan === 'PRO') {
      if (usedBytes + contentLength > lim.capBytesPerPeriod) {
        return res.status(429).json({ error: 'pro_bytes_quota_exceeded', limitBytes: lim.capBytesPerPeriod, usedBytes })
      }
    } else {
      // PROMAX -> sin límite de bytes (si quieres, agrega uno opcional)
    }

    // Construir key con prefijo por plan (facilita lifecycle)
    const ext = extOf(filename)
    const day = new Date().toISOString().slice(0, 10)
    const key = `mt/${plan}/${day}/${randomUUID()}${ext ? '.' + ext : ''}`

    // Crear registro link (activo) con expires_at
    const ttlDays = lim.ttlDays
    const client = await pool.connect()
    try {
      await client.query('BEGIN')
      await client.query(
    ...
    // Presign PUT (para subir) y GET (descarga)
    ...
    const getCmd = new GetObjectCommand({
      Bucket: S3_BUCKET, Key: key, ResponseContentDisposition: `attachment; filename="${filename}"`
    })
    // El link de descarga firmado vive (máx) 24h — el objeto vive físicamente TTL del plan
    const downloadUrl = await getSignedUrl(s3, getCmd, { expiresIn: num(DOWNLOAD_URL_TTL_SECONDS_MAX) })
    ...
    res.json({
      ...
      downloadUrl,
      expiresInSeconds: ttlDays * 24 * 3600 // vida lógica del link (objeto)
    })
  } catch (e) {
    console.error(e)
