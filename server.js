// Mixtli Transfer — Backend v2.14.6-lock+wt-migrate (combo)
...
async function migrateDb () {
  ...
  // Rename legacy if exists
  await safeExec(`ALTER TABLE IF EXISTS packages RENAME TO paquetes;`)
  ...
  await pool.query(`
    CREATE TABLE IF NOT EXISTS package_files (
      id BIGSERIAL PRIMARY KEY,
      package_id UUID REFERENCES paquetes(id) ON DELETE CASCADE,
      key TEXT NOT NULL,
      filename TEXT,
      size BIGINT,
      content_type TEXT
  ...
  await safeExec('CREATE INDEX IF NOT EXISTS paquetes_expires_idx ON paquetes(expires_at);')

  await safeExec(`CREATE TABLE IF NOT EXISTS package_downloads (
    id BIGSERIAL PRIMARY KEY,
    package_id UUID REFERENCES paquetes(id) ON DELETE CASCADE,
    ip TEXT,
    user_agent TEXT,
...
async function purgeExpiredPackages () { try { await pool.query('DELETE FROM paquetes WHERE expires_at IS NOT NULL AND expires_at < now()') } catch {} }
...
function sanitize(ep){ return String(ep||'').replace(/\\/+$/,'') }
async function buildPublicUrl (key) {
  if (PUBLIC_BASE_URL) return `${sanitize(PUBLIC_BASE_URL)}/${key}`
  const endpoint = sanitize(S3_ENDPOINT)
...
function absoluteZipUrl (id) { return `${sanitize(BACKEND_PUBLIC_ORIGIN)}/api/pack/${id}/zip` }
...
app.get('/api/health', (_req,res)=>res.json({ ok:true, time:new Date().toISOString(), ver:'2.14.6-lock+wt-migrate', channel:'sms-only' }))
...
app.get('/api/diag', (req,res)=>{
  ...
  res.json({ ok:true, node:process.version, ver:'2.14.6-lock+wt-migrate', cors_origins:ORIGINS, backend_origin: BACKEND_PUBLIC_ORIGIN })
})
...
// PACKAGES
function planLimitVals(userPlan){
  const lim = planLimits(userPlan||'FREE'); return lim
}
...
app.post('/api/pack/create', requireAuth, async (req,res)=>{
  try{
    ...
    const lim = planLimitVals(userRow?.plan || 'FREE')
    ...
    const r = await pool.query(
      `INSERT INTO paquetes (owner_uid, title, total_size, expires_at, password_hash, password_salt, max_downloads, max_total_mb)
    ...
    const url = relative ? sharePath : ((PUBLIC_BASE_URL||'') ? (sanitize(PUBLIC_BASE_URL)+sharePath) : sharePath)
...
app.get('/api/pack/:id', async (req,res)=>{
  try{
    ...
    const p = await pool.query('SELECT * FROM paquetes WHERE id=$1', [id])
...
app.get('/share/:id', async (req,res)=>{
  try{
    ...
    const btn = needPwd ? `<button id="dlAll" style="padding:8px 12px;background:#34d399;color:#001;border:0;border-radius:8px">Descargar todo (ZIP)</button>`
...
async function guardPackageAccess(req,res,next){
  try{
    const id = req.params.id
    const p = (await pool.query('SELECT * FROM paquetes WHERE id=$1', [id])).rows[0]
...
app.get('/api/pack/:id/zip', guardPackageAccess, async (req,res)=>{
  try{
    const p = req._pkg; const id = p.id
    const f = await pool.query('SELECT key, filename, size FROM package_files WHERE package_id=$1 ORDER BY id', [id])
    ...
    archive.on('end', async()=>{ try { await pool.query('UPDATE paquetes SET download_count = download_count + 1, last_download_at=now() WHERE id=$1', [id]) } catch {} })
