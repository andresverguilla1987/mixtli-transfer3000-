...
function parseAllowedOrigins() {
  try {
    ...
    console.error('ALLOWED_ORIGINS mal formateado. Usa JSON.');
    return [];
  }
}
...
app.use(cors({
  ...
  allowedHeaders: ['Content-Type','x-mixtli-token','authorization','x-amz-acl','x-amz-content-sha256','x-amz-date','x-amz-security-token']
}));
...
app.get('/api/debug-cred', (_req, res) => {
  ...
  res.json({ ok:true, accessKeyId_len: id? id.length:0, accessKeyId_preview: id? `${id.slice(0,4)}...${id.slice(-4)}`:null, endpoint:S3_ENDPOINT, bucket:BUCKET });
});
...
app.listen(PORT, () => console.log('Mixtli Transfer 3000 v2.4.1 listening on', PORT));
