...
app.post('/api/package/create', authOptional, async (req,res)=>{
  try{
    ...
    let durationDays = req.body?.durationDays
    const ttlDays = plan==='PRO'? PROCFG.ttlDays : (plan==='PROMAX'? PMCFG.ttlDays : clamp(num(durationDays||FREECFG.ttlDefaultDays), FREECFG.ttlDefaultDays, FREECFG.ttlMaxDays))
...
// presign per file inside package
app.post('/api/package/presign', authOptional, async (req,res)=>{
  try{
    ...
    const { rows:pk } = await pool.query(`SELECT id, plan, expires_at FROM packages WHERE id=$1`, [packageId])
    ...
    await pool.query(
      `INSERT INTO package_items (package_id,key,filename,content_type,size_bytes)
       VALUES ($1,$2,$3,$4,$5)`,
...
// counted redirect to signed GET for item
...
// simple share page (HTML) with QR + list
app.get('/dl/:id', async (req,res)=>{
  try{
    const id = req.params.id
    const base = (process.env.PUBLIC_BASE_URL || '').replace(/\/+$/,'') // optional
    ...
    const left = (meta.download_limit==null)? '∞' : (meta.download_limit - meta.downloads)
    ...
    const rows = items.map(it=>`<li><a href="/api/dl/${id}/${it.id}">${it.filename}</a> <small>(${humanSize(it.size_bytes)})</small></li>`).join('')
    res.setHeader('Content-Type','text/html; charset=utf-8')
    res.end(`<!doctype html>
<html><head><meta charset="utf-8"/><meta name="viewport" content="width=device-width,initial-scale=1"/><title>Mixtli · ${id}</title>
<style>
:root{ color-scheme:dark; } body{font-family:system-ui,Segoe UI,Roboto,Arial;background:#0b0b0c;color:#e6e6e6;margin:0}
.container{max-width:760px;margin:24px auto;padding:0 16px}
.card{background:#121212;border:1px solid #2b2b2b;border-radius:14px;padding:16px}
.row{display:flex;gap:18px;align-items:center;flex-wrap:wrap}
h1{font-size:18px;margin:0 0 4px}
small{opacity:.8}
ul{margin:12px 0 0 18px}
.pill{font-size:12px;padding:4px 8px;background:#1c1c1c;border:1px solid #333;border-radius:999px}
...
<body><div class="container">
  <div class="row" style="justify-content:space-between;margin:18px 0">
    <div><h1>Tu paquete está listo</h1>
      <small>ID: ${id} · expira: ${ttlText}</small><br/>
      <small>Descargas restantes: ${left}</small>
    </div>
    <div class="card"><img src="${qr}" alt="QR" width="200" height="200"/></div>
  </div>
  <div class="card">
    <b>Archivos</b>
    <ul>${rows||'<li>Vacío</li>'}</ul>
    <p style="margin-top:12px"><a href="${selfUrl}">Copiar link</a></p>
  </div>
</div></body></html>`)
...
// ---------- Boot ----------
await initDb()
app.listen(PORT, ()=> console.log('MixtliTransfer3000 v2.1 on :' + PORT))
